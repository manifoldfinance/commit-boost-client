...
impl RegistrationCache {
    ...
    pub fn needs_registration(&mut self, pubkey: &BlsPublicKey, fee_recipient: &Address, gas_limit: u64) -> bool {
        match self.registrations.get(pubkey) {
            Some(entry) => {
                ...
                let params_changed = entry.fee_recipient != *fee_recipient || entry.gas_limit != gas_limit;
    ...
    pub fn mark_registered(&mut self, pubkey: BlsPublicKey, fee_recipient: Address, gas_limit: u64) {
        self.registrations.insert(pubkey, RegistrationEntry {
            fee_recipient,
            gas_limit,
    ...
    fn cleanup_expired(&mut self) {
        ...
        self.registrations.retain(|_, entry| {
            entry.registered_at.elapsed() <= REGISTRATION_TTL
        });
        let removed = initial_size - self.registrations.len();
        if removed > 0 {
            debug!(removed_entries = removed, remaining_entries = self.registrations.len(), "Cleaned up expired registration cache entries");
        }
    }
