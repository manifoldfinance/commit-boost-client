...
use cb_tests::{
    ...
    utils::{
        generate_mock_relay, generate_mock_relay_with_batch_size, get_local_address,
...
async fn test_parallel_relay_submissions_select_ok() -> Result<()> {
    ...
    let successful_relays =
        mock_states.iter().filter(|s| s.received_register_validator() > 0).count();
    assert!(successful_relays >= 1, "At least one relay should have received registrations");

    info!("Select OK test completed in {:?}, {} relays participated", elapsed, successful_relays);
...
async fn test_relay_failure_recovery_with_timeout() -> Result<()> {
    ...
    assert!(
        mock_state1.received_register_validator() > 0,
        "Working relay should receive registrations"
    );

    // Verify that relay 2 was attempted (even though it failed)
    assert!(
        mock_state2.received_register_validator() > 0,
        "Failed relay should have been attempted"
    );
...
async fn test_relay_coordination_performance_measurements() -> Result<()> {
    ...
    for (i, batch_size) in batch_sizes.iter().enumerate() {
        ...
        let pubkey = blst_pubkey_to_alloy(&signer.sk_to_pk());
        let relay =
            generate_mock_relay_with_batch_size(pbs_port + i as u16 + 1, pubkey, *batch_size)?;
        relays.push(relay);
