...
impl TestConfig {
    ...
    fn validate(self) -> Result<CommitBoostConfig> {
        ...
        if let Some(ref modules) = self.config.modules {
            ...
            matrix
                .validate_module_list(&module_ids)
...
fn test_incompatible_modules_fail_validation() {
    ...
    assert!(
        ...
        "Error should mention both conflicting modules: {}",
        error_msg
    );
}
...
fn test_conflict_matrix_integration() {
    ...
    let compatible_config = vec!["pbs".to_string(), "signer".to_string(), "metrics".to_string()];
    ...
    let incompatible_config = vec!["custom_pbs".to_string(), "pbs".to_string()];
...
fn test_edge_cases() {
    ...
    assert!(matrix.validate_module_list(&[]).is_ok(), "Empty module list should be valid");

    // Test single module
    let single_module = vec!["pbs".to_string()];
    assert!(matrix.validate_module_list(&single_module).is_ok(), "Single module should be valid");

    // Test unknown modules (should default to compatible)
    let unknown_modules = vec!["unknown_module_1".to_string(), "unknown_module_2".to_string()];
    assert!(
...
fn test_metric_namespace_separation() {
    ...
    let module_metrics =
        vec![("pbs", "cb_pbs_"), ("signer", "cb_signer_"), ("metrics", "cb_metrics_")];
...
fn test_real_world_configuration() {
    ...
    assert!(config.is_ok(), "Realistic configuration should be valid: {:?}", config.err());
}
...
async fn test_module_startup_sequence() {
    ...
    let modules = vec![("pbs", 8080), ("signer", 8081), ("metrics", 9090)];
...
async fn test_configuration_validation_pipeline() {
    ...
    config.modules = Some(vec![create_test_module("pbs"), create_test_module("custom_pbs")]);
    assert!(validate_config(&config).is_err());
