...
mod tests {
    ...
    fn test_module_id_uniqueness() {
        ...
        if let Some(modules) = config.modules {
            for module in modules {
                assert!(seen_ids.insert(module.id.clone()), "Duplicate module ID: {}", module.id);
            }
        }
    }
    ...
    fn test_port_conflicts() {
        ...
        assert!(
            ...
            "PBS port {} conflicts with an existing service",
            config.pbs.pbs_config.port
        );

        // Check signer port if exists
        if let Some(signer) = config.signer {
            assert!(used_ports.insert(signer.port), "Signer port {} already in use", signer.port);
        }
    ...
    fn test_known_incompatible_combinations() {
        ...
        for (module_a, module_b) in incompatible_pairs {
            assert!(
                ...
                "Modules '{}' and '{}' should be incompatible",
                module_a,
                module_b
            );
        }
    ...
    fn test_known_compatible_combinations() {
        ...
        let compatible_pairs = vec![("pbs", "signer"), ("pbs", "metrics"), ("signer", "metrics")];

        for (module_a, module_b) in compatible_pairs {
            assert!(
                matrix.can_coexist(module_a, module_b),
                "Modules '{}' and '{}' should be compatible",
                module_a,
                module_b
            );
        }
    ...
    fn test_conflict_matrix_symmetry() {
        ...
        let test_pairs =
            vec![("pbs", "signer"), ("pbs_relay_a", "pbs_relay_b"), ("custom_pbs", "pbs")];
    ...
    fn test_single_module_configuration() {
        ...
        let single_modules =
            vec![vec!["pbs".to_string()], vec!["signer".to_string()], vec!["metrics".to_string()]];

        for module_list in single_modules {
            let result = matrix.validate_module_list(&module_list);
            assert!(result.is_ok(), "Single module '{}' should be valid", module_list[0]);
        }
    }
}
